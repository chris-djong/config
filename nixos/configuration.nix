...
{ imports =
  ...
  # Enable the X11 windowing system. You can disable this if you're only using the 
  # Wayland session.
  services.xserver = {
    enable = true;
    desktopManager = {
  ...
  services.pipewire = {
    ...
    # use the example session manager (no others are packaged yet so this is enabled 
    # by default, no need to redefine it in your config for now)
